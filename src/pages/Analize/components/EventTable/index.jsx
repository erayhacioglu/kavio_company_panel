...
import { getEventTypeLabel } from '../../mockData';
...
const EventsTable = ({ events = [] }) => {
  ...
  // events prop'u değiştiğinde veriyi güncelle
  useEffect(() => {
    setData(events);
  }, [events]);

  // TanStack Table için columns tanımlıyoruz
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      header: 'Grup',
      cell: ({ getValue }) => {
        return <span style={{ color: 'var(--textSecondary)' }}>{getValue()}</span>;
      },
      enableSorting: true,
    },
    {
    ...
    },
    {
      ...
      header: 'Ziyaretçi Tipi',
      cell: ({ getValue }) => {
        const type = getValue();
        return (
          <span style={{ 
            color: type === 'registered' ? 'var(--success)' : 'var(--textSecondary)',
            fontWeight: type === 'registered' ? '500' : '400'
          }}>
            {type === 'registered' ? '✓ Kayıtlı' : 'Misafir'}
          </span>
        );
      },
      enableSorting: true,
